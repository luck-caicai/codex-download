...
各系统卡片也提供中文工具下载：Windows 为“中文设置脚本”，Mac 为“中文显示修复”。脚本压缩包随网页一起托管，不经过网盘；Mac 两种芯片共用一个压缩包。
...
验证包括：20 项修补检查（开关缺省 / 关闭 / 开启、真实翻译加载函数配合模拟运行时、归档偏移与分块哈希、原文件保留、未知结构拒绝）和 12 项 Windows 流程检查。当前版本支持的修补点为两处。真实界面是否生效仍需在目标电脑确认。
